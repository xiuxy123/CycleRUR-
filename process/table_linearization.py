...
def linearize_table_with_index(table, cell_indices, table_page_title, table_section_title, order_cell=False):
    ...
    * highlighted cells (<cell>)
    ...
    if table_page_title:
        # table_str += " [S] [TITLE]" + " [O] " + table_page_title
        table_str = " [S] " + table_page_title
        type_ids += [1 for _ in range(len(table_str))]
    # if table_section_title:
    #     table_str += " [T] " + table_section_title + " "
    ...
    for item in subtable:
        ...
        for header, index in enumerate(headers):
            item_str += table_str + " [P] " + headers[0]["value"]
            if len(index) > 1:
                break

        # The value of the cell.
        item_str += " [O] " + cell["value"]
    ...
    return final_str, type_ids, row_ids, col_ids
