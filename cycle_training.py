...
#
# # 下载数据包并指定目录
# nltk.data.path.append('/home/admin1/miniconda3/envs/wxbtorch13/nltk_data')
#
# # 下载所需的 NLTK 数据包
# nltk.download('punkt', download_dir='/home/admin1/miniconda3/envs/wxbtorch13/nltk_data')
# nltk.download('wordnet', download_dir='/home/admin1/miniconda3/envs/wxbtorch13/nltk_data')
# nltk.download('omw-1.4', download_dir='/home/admin1/miniconda3/envs/wxbtorch13/nltk_data')

# '--gpu_device','0',
#         '--num_epochs','1',
# '--output_dir', 'output',
# '--data2text_model', 'model1/t5',
# '--text2data_model', 'model2/t5',
# '--text_file', 'data/webnlg/t2d_train.txt',
# '--data_file', 'data/webnlg/d2t_train.txt',
#
# sys.argv=[
#
#
...
#           '--scorer_model','',
#           '--data2text_validation_file','data/webnlg/d2t_dev.tsv',
#           '--text2data_validation_file','data/webnlg/t2d_dev.tsv',
#           '--data2text_test_file','data/webnlg/d2t_test.tsv',
#           '--text2data_test_file','data/webnlg/t2d_test.tsv']
...
## Data
parser.add_argument("--text_file", default="data/webnlg/t2d_train_0.txt", type=str,
                    help="Text used for cycle training (text-data-text cycle)")
parser.add_argument("--data_file", default="data/webnlg/d2t_train_0.txt", type=str,
...
parser.add_argument("--data2text_validation_file", default="data/webnlg/d2t_dev.csv", type=str,
                    help="The development set of the data2text task")
parser.add_argument("--text2data_validation_file", default="data/webnlg/t2d_dev.csv", type=str,
                    help="The development set of the text2data task")

parser.add_argument("--do_generate", action='store_true', default=True,
                    help="Whether to run generation for the evaluation of the dev set")

parser.add_argument("--do_test", action='store_true', default=True,
                    help="Whether to run eval on the test set")
parser.add_argument("--data2text_test_file", default="data/webnlg/d2t_test.csv", type=str,
                    help="The test set of the data2text task")
parser.add_argument("--text2data_test_file", default="data/webnlg/t2d_test.csv", type=str,
...
## LOSS
parser.add_argument("--rankloss", action='store_true', default=False,
                    help="Whether to run eval on the test set")
...
cached_files = glob.glob(os.path.join(args.output_dir, '*text*'))
# reomve_file(path)
for cache_path in cached_files:
    if cache_path not in [args.load_data2text_model, args.load_text2data_model]:
        remove_files(cache_path)
...
def train_one_direction(model1,model2,data_loader,num_training_steps,optimizer,lr_scheduler,inter_type):
    ...
    wordloss = Wordsloss(encoder=model2.encoder, tokenizer=tokenizer, args=args)
    ls_epsilon = 0.1
    smoothloss = LabelSmoothingLoss(  # 标签平滑损失
        ls_epsilon, model2.config.vocab_size, ignore_index=0).to(model1.device)
    ...
    for batch in data_loader:
        ...
        with torch.no_grad():
            ...
            # intermediate_outputs= intermediate_outputs.view(int(intermediate_outputs.shape[0] / args.num_beams), args.num_beams, -1)
        ...
        # processed_labels[processed_labels == -100] = 0
        # ce_loss = smoothloss(logits.view(-1, logits.size(-1)), processed_labels.view(-1), average=False).sum()
        # num_tokens = processed_labels.view(-1).ne(0).sum()
        # ce_loss /= num_tokens
        # loss = ce_loss
        ...
        # wordloss
            if not are_last_three_zeros(arr=wordloss_list):
                # try:
                    decoded_negative_outputs = tokenizer.batch_decode(intermediate_outputs,
...
metric_meteor = load_metric("meteor",trust_remote_code=True)
metric_bleu = load_metric("bleu",trust_remote_code=True)
...
def eval_model(model,data_loader,test_mode=False):
    ...
    if args.do_generate or test_mode:
        ...
        # hyp = dict(zip(range(len(generated_texts)), [[x.lower()] for x in generated_texts]))
        # ref = dict(zip(range(len(target_texts)), [[x.lower()] for x in target_texts]))
        # ret = bleu.compute_score(ref, hyp)
        #
        # resulting_metrics['meteor'] = meteor.compute_score(ref, hyp)[0]
        # resulting_metrics['bleu'] = ret[0]
        # # resulting_metrics['bertscore'] = np.mean(metric_bertscore.compute(predictions = bertscore_p, references=bertscore_g, lang='en')['f1'])
        # rouge_result = rouge.compute_score(ref, hyp)
        # resulting_metrics['rougeL'] = rouge_result[0]
        # resulting_metrics['Cider'] = cider.compute_score(ref, hyp)[0]
        meteor_p = []
        meteor_g = []
        bleu_p = []
        bleu_g = []
        ...
        # resulting_metrics['meteor'] = meteor.compute_score(meteor_g, meteor_p)[0]
        # ret = bleu.compute_score(meteor_g, meteor_p)
        # resulting_metrics['bleu'] = ret[0]
        # rouge_result = rouge.compute_score(meteor_g, meteor_p)
        # resulting_metrics['rougeL'] = rouge_result[0]
        # resulting_metrics['Cider'] = cider.compute_score(meteor_g, meteor_p)[0]
...
if args.do_train:
    ...
    for epoch in range(args.num_epochs):
        ...
        #
        #     # before_model_path = os.path.join(args.output_dir, 'text2data-' + str(epoch - 1))
        #     # # 删除之前保存的权重参数
